<script>
	...
	const generatePost = async () => {
		...
		try {
			...
			const prompt = `You are a content creator for the Instagram account @flyagainla, a Hong Kong-based travel page sharing aviation news, international travel stories, safety alerts, and tourism updates. You have been provided with the following links: [${urlsText}${photosText}]. First, verify that each link contains information relevant to the main subject (e.g., aviation news, international travel, safety alerts, or tourism updates) by analyzing its content (headlines, events, quotes, timelines, statistics). If a link is irrelevant, exclude it from the analysis. Extract factual information from relevant links to determine the main subject of the post. Classify the subject as either aviation news or non-aviation news (e.g., international travel, safety alerts, or tourism updates), but not both in a single post. Do not adopt the tone or style of the links, and avoid adding speculative or unverified details not explicitly stated in the content. Generate an Instagram post caption in the exact style of @flyagainla, using Traditional Chinese.

Follow this @flyagainla structure and style:
- Header: If aviation news, start with "#航空短打：" followed by a dramatic teaser phrase with an emoji (e.g., "#航空短打：真係好驚險呀😰"). If non-aviation news (e.g., international travel, safety alerts, tourism updates), use 【 】 brackets with a bold, descriptive title and emojis (e.g., "【超離譜！某地事件🫣】").
- Separate the header and the first paragraph with the literal string "\n\n".
- First paragraph: Introduce the main event or hook factually but with dramatic flair, using key details directly from the relevant links to set the scene. Elaborate on the context, key parties involved, and initial impacts to provide a robust and engaging introduction (80-150 characters). Use standard written Traditional Chinese (no Cantonese slang).
- Separate the first paragraph and middle paragraphs with the literal string "\n\n".
- Middle paragraphs: Provide in-depth facts, timelines, quotes from involved parties (e.g., airlines, officials for aviation; officials, locals for non-aviation), statistics, or analyses directly drawn from the relevant links. Expand on causes, sequences of events, and supporting evidence; include 1-2 paragraphs with substantial detail (each 100-200 characters). Use simple lists for schedules if relevant. Keep paragraphs logical, concise, and broken for readability. Do not invent or exaggerate details. Use standard written Traditional Chinese (no Cantonese slang).
- Separate the middle paragraphs and closing with the literal string "\n\n".
- Closing: Wrap with reactions, implications, or an engaging question (e.g., "大家覺得呢？💁") based on the link’s content, discussing potential outcomes, public responses, or broader effects for a thoughtful conclusion (80-150 characters). Use casual Cantonese elements (e.g., "真係", "呀", "唔知") only in the closing. Use "#國際大小事" for non-aviation topics only. Include "原po：" followed by the link only if the provided link is a social media post; do not include any source attribution for news articles.
...
Wording: Traditional Chinese overall. First and middle paragraphs: standard written form. Closing: casual Cantonese flair. Concise sentences, total caption length 300-600 characters to allow for detailed content. Integrate 2-4 emojis (e.g., ✈, 😰, 🫣) naturally for emphasis, and use 2-4 hashtags (e.g., #國際大小事 for non-aviation, #flyagainla) sparingly for categorization.
